...
function IndexPage({ source , posts  }) {
    const { scope: { title , description , url , date  } ,  } = source;
...
function getStaticPaths() {
    const postPaths = path__WEBPACK_IMPORTED_MODULE_4___default().join(process.cwd(), "posts/developer");
...
async function getStaticProps({ params  }) {
    const allPostsPaths = path__WEBPACK_IMPORTED_MODULE_4___default().join(process.cwd(), "posts/developer");
    const globPosts = glob__WEBPACK_IMPORTED_MODULE_1___default().sync("**/*.mdx", {
        cwd: allPostsPaths
    });
    ...
    const postsPath = path__WEBPACK_IMPORTED_MODULE_4___default().join(process.cwd(), "posts", "developer", params.slug + ".mdx");
...
var __webpack_exports__ = __webpack_require__.X(0, [505,664,374], () => (__webpack_exec__(8970)));
